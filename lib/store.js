...
const extendSewaGroup = (gid, additionalTime, _dir) => {
    const groupIndex = _dir.findIndex(group => group.id === gid);
    if (groupIndex !== -1) {
        _dir[groupIndex].expired += toMs(additionalTime);
        fs.writeFileSync('./database/sewa.json', JSON.stringify(_dir, null, 2));
    }
};
...
const expiredCheck = (jerofc, _dir) => {
    setInterval(() => {
        let position = null;
