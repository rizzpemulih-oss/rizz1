...
exports.smsg = (conn, m, store) => {
  ...
  if (m.key) {
    ...
    m.chat = conn.decodeJid(m.key.remoteJid); // normalisasi jid
    let remoteJid = m.key.remoteJid;

    // handle @lid
    if (remoteJid.endsWith("@lid")) {
      if (m.key.senderPn) {
        remoteJid = m.key.senderPn;
      } else {
        return m; // keluar aman, jangan return undefined
      }
    }

    m.fromMe = m.key.fromMe;
    m.isGroup = m.chat.endsWith("@g.us"); // ✅ boolean, bukan string

    // kalau group → ambil participant asli
    if (m.isGroup) {
