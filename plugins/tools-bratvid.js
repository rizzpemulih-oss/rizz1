...
let handler = async (m, { jerofc, text, reply, prefix, command }) => {
  ...
  try {
    for (let i = 0; i < words.length; i++) {
      ...
      const apiUrl = `https://jerofc.my.id/api/tools/brat?text=${encodedText}&apikey=${jerapi}`; // Use encoded currentText
      ...
      const res = await axios
        ...
        .catch((e) => {
          console.error(
            `Error fetching frame for text "${currentText}":`,
            e.response ? e.response.data.toString() : e.message
          );
