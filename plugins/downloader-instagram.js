...
let handler = async (m, { jerofc, text, reply, command, prefix, quoted }) => {
	if (!text) return reply(`Please enter the url\n\nExample: ${prefix + command} https://www.instagram.com/reel/DFcoYF5STbs/?igsh=MTJsOXJqNHN1bHgzcQ==`)
	try {
		const { igdl } = require('jer-api')
		let api = await fetch(`https://jerofc.my.id/api/igdl?url=${text}&apikey=${jerapi}`);
